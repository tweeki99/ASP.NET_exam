...
namespace Exam.Services
{
    public class AuthService
    {
        ...
        public async Task<string> Registration(string login, string password)
        {
            ...
            var existingUser = await context.Users.FirstOrDefaultAsync(x => x.Username == login);

            if (existingUser != null)
                return null;

            context.Users.Add(new User { Username = login, Password = password });

            await context.SaveChangesAsync();

            return "Пользователь успешно зареган";
        }
    }
}
